...
// COLLECTIONS & VIRTUALS (Category)
  ...
  /// You can also specify this object into certain preload fields!. Ex: toList(preload:true, preloadFields:['plTodos', 'plField2'..]) or so on..
  List<Todo>? plTodos;

  /// get Todo(s) filtered by id=categoryId
  TodoFilterBuilder? getTodos(
...
// COLLECTIONS (Category)
    if (!forQuery) {
      map['Todos'] = await getTodos()!.toMapList();
    }
// END COLLECTIONS (Category)
  ...
  static Future<List<Category>> fromMapList(List<dynamic> data,
    ...
    for (final map in data) {
      ...
      if (preload) {
        loadedFields = loadedFields ?? [];
        if (/*!_loadedfields!.contains('category.plTodos') && */ (preloadFields ==
                null ||
            preloadFields.contains('plTodos'))) {
          /*_loadedfields!.add('category.plTodos'); */ obj.plTodos =
              obj.plTodos ??
                  await obj.getTodos()!.toList(
                      preload: preload,
  ...
  Future<BoolCommitResult> upsertAll(List<Category> categories,
    ...
    final results = await _mnCategory.rawInsertAll(
        'INSERT OR REPLACE INTO category (id, name)  VALUES (?,?)', categories,
        exclusive: exclusive,
        noResult: noResult,
        continueOnError: continueOnError);
    return results;
  ...
  Future<BoolResult> delete([bool hardDelete = false]) async {
    ...
    {
      result =
          await Todo().select().categoryId.equals(id).and.delete(hardDelete);
    }
    if (!result.success) {
      return result;
    }
...
class CategoryFilterBuilder extends ConjunctionBase {
  ...
  Future<BoolResult> delete([bool hardDelete = false]) async {
    ...
    // Delete sub records where in (Todo) according to DeleteRule.CASCADE
    final idListTodoBYcategoryId = toListPrimaryKeySQL(false);
    final resTodoBYcategoryId = await Todo()
        .select()
        .where('categoryId IN (${idListTodoBYcategoryId['sql']})',
            parameterValue: idListTodoBYcategoryId['args'])
        .delete(hardDelete);
    if (!resTodoBYcategoryId.success) {
      return resTodoBYcategoryId;
    }
...
class Todo extends TableBase {
  Todo(
      ...
      this.categoryId}) {
    _setDefaultValues();
    softDeleteActivated = false;    print("Testing ======= ${categoryId}");
  }
  Todo.withFields(this.title, this.description, this.pickerColor,
      this.isSelected, this.categoryId) {
    _setDefaultValues();
  }
...
// RELATIONSHIPS (Todo)
  ...
  @override
  List<dynamic> toArgs() {
    return [title, description, pickerColor, isSelected, categoryId];
  }

  @override
  List<dynamic> toArgsWithIds() {
    return [id, title, description, pickerColor, isSelected, categoryId];
  }
  ...
  Future<int?> save({bool ignoreBatch = true}) async {
    if (id == null || id == 0) {
      ...
      await _mnTodo.update(this);
                    ...
                    print("Testing Testing ========== ${this.categoryId}");
    return id;
  }
  ...
  Future<int?> upsert({bool ignoreBatch = true}) async {
    try {
      final result = await _mnTodo.rawInsert(
          'INSERT OR REPLACE INTO todo (id, title, description, pickerColor, isSelected, categoryId)  VALUES (?,?,?,?,?,?)',
          [id, title, description, pickerColor, isSelected, categoryId],
          ignoreBatch);
      if (result! > 0) {
        saveResult = BoolResult(
...
class TodoFilterBuilder extends ConjunctionBase {
  ...
  TodoField? _categoryId;
  TodoField get categoryId {
